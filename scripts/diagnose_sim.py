...
def parse_log(log: str) -> list[TaskRecord]:
    ...
    for h, proc in failed_processes.items():
        if h not in by_hash:
            by_hash[h] = TaskRecord(
                hash_prefix=h, process=proc,
                status_raw="(no progress line seen)",
                failed=True, ignored=True, completed=False,
            )
...
def _list_subdirs(s3, bucket: str, prefix: str, max_keys: int = 1000) -> list[str]:
    ...
    try:
        ...
        if code not in _s3_errors_seen:
            ...
            if code in ("InvalidToken", "ExpiredToken", "TokenRefreshRequired",
...
def show_experiment_tree(
    s3, bucket: str, experiment_id: str, max_entries: int = 50
) -> None:
    """
...
def _make_s3_client(region: str | None, profile: str | None):
    ...
    if creds is None:
        print(
            "  ⚠  boto3 found no credentials. Run `aws sso login` (or export "
            "AWS_PROFILE) and retry."
        )
    else:
...
def diagnose(
    ...
    *,
    ...
    # Resolve region with bucket-name heuristic FIRST: if the bucket looks
    # like a Stanford GovCloud shared-stack bucket, force us-gov-west-1
    # regardless of what settings say. Otherwise the storage_s3_region
    # default (us-east-1 in config.py) leaks in and gives InvalidToken.
    if region is None and "smsvpctest" in bucket:
        region = "us-gov-west-1"
        print(f"  (auto-selected region {region} based on bucket name)")
    if region is None:
        region = (
    ...
    for t in interesting:
        ...
        # Also list what's in the work dir so we can see whether output
        # files (e.g. kb_hash.txt, metadata.json) made it.
    ...
    # If nothing resolved, dump the actual S3 layout so the user can see
    ...
    # Cross-check: do the parca kb paths actually have contents?
    ...
    # Last-resort: dump everything under the experiment prefix so we can
    # see what DID land on S3 even if the expected paths are empty.
    ...
    try:
        for page in paginator.paginate(
            ...
            for obj in page.get("Contents", []) or []:
                ...
                # Group by the first 3 path segments after the prefix
                key_parts = obj["Key"].split("/")
                if len(key_parts) >= 5:
                    group = "/".join(key_parts[:5])
                else:
    ...
    if count == 0:
        ...
        # Try the other observed upload location
