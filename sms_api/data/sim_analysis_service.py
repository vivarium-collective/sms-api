...
def connect_ssh(func) -> Any:
    ...
    async def wrapper(self: "AnalysisService", **kwargs) -> Any:
        try:
            logger.info(f"IS SSH CONNECTED ALREADY?: {self.ssh.connected}")
            if self.ssh.connected:
                logger.info("WEIRD, SSH is READY CONNECTED. THAT SHOULDNT BE.")
                raise RuntimeError("WEIRD, SSH is READY CONNECTED. THAT SHOULDNT BE.")
