...
from sms_api.notebook.initial_conditions import (
    calculate_cell_mass,
...
class LoadSimData:
    def __init__(
        ...
        **kwargs,
        ...
        self.submass_indices = {
            f"massDiff_{submass}": idx for submass, idx in self.sim_data.submass_name_to_index.items()
        }
        ...
        if mar_regulon:
            ...
            curr_size = self.sim_data.internal_state.unique_molecule.unique_molecule_definitions["promoter"][
                "bound_TF"
            ][1]
            self.sim_data.internal_state.unique_molecule.unique_molecule_definitions["promoter"]["bound_TF"] = (
                "?",
                curr_size + 2,
            )
            ...
            treg_alias.delta_prob["deltaI"] = np.concatenate([treg_alias.delta_prob["deltaI"], new_deltaI])
            treg_alias.delta_prob["deltaJ"] = np.concatenate([treg_alias.delta_prob["deltaJ"], new_deltaJ])
            treg_alias.delta_prob["deltaV"] = np.concatenate([treg_alias.delta_prob["deltaV"], new_deltaV])
            ...
            free_30s_mass = np.array(bulk_data[bulk_data["id"] == "CPLX0-3953[c]"][0][1])
            ...
            eq_alias._stoichMatrixJ = np.concatenate([eq_alias._stoichMatrixJ, np.array([stoich_matrix_shape[1]] * 3)])
            eq_alias._stoichMatrixV = np.concatenate([eq_alias._stoichMatrixV, np.array([-1, -1, 1])])
            ...
            eq_alias.ids_complexes = [
                eq_alias.molecule_names[i] for i in np.where(np.any(eq_alias.stoich_matrix() > 0, axis=1))[0]
            ]
            ...
            eq_alias._stoichMatrixMass = np.concatenate([
                ...
                np.array([marR_mass.sum(), tet_mass.sum(), (marR_mass + tet_mass).sum()]),
            ])
        ...
        if isinstance(process_configs, dict):
            ...
            if rnai_data:
                ...
                for i, (srna_id, target_id) in enumerate(zip(self.srna_ids, target_ids)):
                    ...
                    srna_tu_id = cistron_tu_mapping[cistron_data["id"] == srna_id[:-3]].nonzero()[1][0]
                    self.target_tu_ids[i] = cistron_tu_mapping[cistron_data["id"] == target_id[:-3]].nonzero()[1][0]
                    duplex_ACGU[i] = (
                        rna_data["counts_ACGU"][srna_tu_id] + rna_data["counts_ACGU"][self.target_tu_ids[i]]
                    )
                    duplex_mw[i] = rna_data["mw"][srna_tu_id] + rna_data["mw"][self.target_tu_ids[i]]
                    ...
                    duplex_sequences[i, srna_length : srna_length + target_length] = rna_sequences[
                        self.target_tu_ids[i]
                    ][:target_length]
                ...
                ts_alias.exp_free = np.concatenate([ts_alias.exp_free, [0] * n_duplex_rnas])
                ts_alias.exp_ppgpp = np.concatenate([ts_alias.exp_ppgpp, [0] * n_duplex_rnas])
    ...
    def get_monomer_counts_indices(self, names):
        ...
        monomer_ids = np.array([re.split(r"\[.\]", monomer)[0] for monomer in monomer_ids])
    ...
    def get_chromosome_replication_config(self, time_step=1):
        ...
        replisome_trimer_subunit_masses = np.vstack([
            self.sim_data.getter.get_submass_array(x).asNumber(units.fg / units.count)
        ...
        replisome_mass_array = 3 * replisome_trimer_subunit_masses.sum(axis=0) + replisome_monomer_subunit_masses.sum(
            axis=0
        )
    ...
    def get_rna_degradation_config(self, time_step=1):
        ...
        rna_degradation_config = {
            ...
            "mature_rna_cistron_indexes": np.array([cistron_id_to_index[rna_id[:-3]] for rna_id in mature_rna_ids]),
            ...
            "uncharged_trna_indexes": np.array([
                rna_id_to_index[trna_id] for trna_id in transcription.uncharged_trna_names
            ]),
            ...
            * np.concatenate((
            ...
            "is_miscRNA": np.concatenate((
                ...
                np.array([False] * len(transcription.mature_rna_data), dtype=np.int64),
            )),
    ...
    def get_metabolism_redux_config(self, time_step=1):
        ...
        aa_exchange_names = np.array([self.sim_data.external_state.env_to_exchange_map[aa[:-3]] for aa in aa_names])
        ...
        if hasattr(self.sim_data, "ppgpp_ramp"):
            ...
            self.sim_data.growth_rate_parameters.get_ppGpp_conc = self.sim_data.ppgpp_ramp.get_ppGpp_conc
        ...
        metabolism_config = {
            ...
            "removed_aa_uptake": np.array([aa in aa_targets_not_updated for aa in aa_exchange_names]),
    ...
    def get_mass_config(self, time_step=1):
        ...
        for molecule_id in bulk_ids:
            molecular_weights[molecule_id] = self.sim_data.getter.get_mass(molecule_id).asNumber(units.fg / units.mol)
    ...
    def get_mass_listener_config(self, time_step=1):
        ...
        mass_config = {
            "cellDensity": self.sim_data.constants.cell_density.asNumber(units.g / units.L),
            "bulk_ids": self.sim_data.internal_state.bulk_molecules.bulk_data["id"],
            "bulk_masses": self.sim_data.internal_state.bulk_molecules.bulk_data["mass"].asNumber(units.fg / units.mol)
    ...
    def get_rna_counts_listener_config(self, time_step=1):
        counts_config = {
            ...
            "rRNA_indexes": np.where(self.sim_data.process.transcription.rna_data["is_rRNA"])[0],
        ...
        counts_config["mRNA_TU_ids"] = counts_config["all_TU_ids"][counts_config["mRNA_indexes"]]
        counts_config["rRNA_TU_ids"] = counts_config["all_TU_ids"][counts_config["rRNA_indexes"]]
        counts_config["mRNA_cistron_ids"] = counts_config["all_cistron_ids"][counts_config["cistron_is_mRNA"]]
        counts_config["rRNA_cistron_ids"] = counts_config["all_cistron_ids"][counts_config["cistron_is_rRNA"]]
    ...
    def get_monomer_counts_listener_config(self, time_step=1):
        monomer_counts_config = {
            ...
            "two_component_system_molecule_ids": list(self.sim_data.process.two_component_system.molecule_names),
            ...
            "rnap_subunits": self.sim_data.process.complexation.get_monomers(self.sim_data.molecule_ids.full_RNAP),
    ...
    def get_chromosome_structure_config(self, time_step=1):
        ...
        chromosome_structure_config = {
            ...
            "unprocessed_rna_index_mapping": {rna_index: i for (i, rna_index) in enumerate(unprocessed_rna_indexes)},
    ...
    def get_rna_maturation_config(self, time_step=1):
        ...
        rna_id_to_index = {rna_id: i for i, rna_id in enumerate(chain(rna_data["id"], mature_rna_data["id"]))}

        def calculate_delta_nt_counts(main_id, variant_ids):
            main_index = rna_id_to_index[main_id]
            variant_indexes = np.array([rna_id_to_index[rna_id] for rna_id in variant_ids])
    ...
    def get_tf_unbinding_config(self, time_step=1):
        ...
        bulk_ids = self.sim_data.internal_state.bulk_molecules.bulk_data["id"]
        tf_indexes = [np.where(bulk_ids == tf_id + "[c]")[0][0] for tf_id in config["tf_ids"]]
    ...
    def get_ribosome_data_listener_config(self, time_step=1):
        return {
            ...
            "rRNA_is_5S": self.sim_data.process.transcription.cistron_data["is_5S_rRNA"][
                self.sim_data.process.transcription.cistron_data["is_rRNA"]
            ],
            "rRNA_is_16S": self.sim_data.process.transcription.cistron_data["is_16S_rRNA"][
                self.sim_data.process.transcription.cistron_data["is_rRNA"]
            ],
            "rRNA_is_23S": self.sim_data.process.transcription.cistron_data["is_23S_rRNA"][
                self.sim_data.process.transcription.cistron_data["is_rRNA"]
            ],
            "emit_unique": self.emit_unique,
        }
    ...
    def get_bulk_timeline_config(self, time_step=1):
        ...
        return {
            "time_step": time_step,
            "timeline": {time: {("media_id",): media_id} for time, media_id in current_timeline},
        }
    ...
    def generate_initial_state(self):
        ...
        return {
            ...
            "boundary": {"external": {mol: conc * vivunits.mM for mol, conc in current_concentrations.items()}},
        }
