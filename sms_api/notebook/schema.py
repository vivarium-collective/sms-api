...
import numpy as np
from vivarium.core.store import Store  # type: ignore[import-untyped]
from vivarium.core.registry import Serializer  # type: ignore[import-untyped]
        ...
        if "unique_index" in obj.dtype.names:
            if "_entryState" in obj.dtype.names:
                ...
                if len(unique_indices) != len(set(unique_indices)):
                    raise ValueError("All elements in the 'unique_index' field must be unique.")
            else:
...
def create_unique_indices(n_indexes: int, unique_molecules: MetadataArray) -> np.ndarray:
    ...
    unique_indices = np.arange(next_unique_index, int(next_unique_index + n_indexes), dtype=int)
...
def bulk_name_to_idx(names: str | (List | np.ndarray), bulk_names: List | np.ndarray) -> int | np.ndarray:
    ...
    if isinstance(names, np.ndarray) or isinstance(names, list):
        ...
        return np.take(sorter, np.searchsorted(bulk_names, names, sorter=sorter), mode="clip")
    else:
        return np.where(np.array(bulk_names) == names)[0][0]


def bulk_numpy_updater(current: np.ndarray, update: List[Tuple[int | np.ndarray, int | np.ndarray]]) -> np.ndarray:
...
def get_free_indices(result: MetadataArray, n_objects: int) -> Tuple[MetadataArray, np.ndarray]:
    ...
    if n_free_indices < n_objects:
        ...
        free_indices = np.concatenate((free_indices, old_size + np.arange(n_new_entries)))
    ...
    def updater(self, current: MetadataArray, update: Dict[str, Any]) -> MetadataArray:
        ...
        for update_type, update_val in update.items():
            if update_type == "add":
                if isinstance(update_val, list):
                    ...
                    raise ValueError("Add updates must be dictionaries or lists of dictionaries")
                ...
                if isinstance(update_val, list):
                    if len(update_val) == 0:
                        continue
                    elif isinstance(update_val[0], list) or isinstance(update_val[0], np.ndarray):
                        ...
                        raise ValueError(
                            "Delete updates must be lists/arrays of integers OR lists of lists/arrays of integers"
                        )
                elif isinstance(update_val, np.ndarray) and np.issubdtype(update_val.dtype, np.integer):
                    self.delete_updates.append(update_val)
                else:
                    raise ValueError(
                        "Delete updates must be lists/arrays of integers OR lists of lists/arrays of integers"
        ...
        for add_update in self.add_updates:
            ...
            if "unique_index" not in add_update:
                result["unique_index"][free_indices] = np.arange(n_new_molecules) + result.metadata
...
def divide_ribosomes_by_RNA(values: MetadataArray, state: Dict[str, Any]) -> tuple[np.ndarray, np.ndarray]:
    ...
    if n_molecules > 0:
        ...
        degraded_mRNA_indexes = np.unique(mRNA_index[np.logical_not(np.logical_or(d1_bool, d2_bool))])
        ...
        if n_degraded_mRNA > 0:
            ...
            degraded_mRNA_indexes_d2 = np.setdiff1d(degraded_mRNA_indexes, degraded_mRNA_indexes_d1)
...
def divide_domains(state: dict[str, MetadataArray]) -> dict[str, np.ndarray]:
    ...
    domain_index_domains, child_domains = attrs(state["chromosome_domain"], ["domain_index", "child_domains"])
    ...
    d1_all_domain_indexes = get_descendent_domains(d1_domain_index_full_chroms, domain_index_domains, child_domains, -1)
    d2_all_domain_indexes = get_descendent_domains(d2_domain_index_full_chroms, domain_index_domains, child_domains, -1)
...
def divide_by_domain(values: np.ndarray, state: Dict[str, Any]) -> tuple[np.ndarray, np.ndarray]:
...
def divide_RNAs_by_domain(values: MetadataArray, state: Dict[str, Any]) -> tuple[np.ndarray, np.ndarray]:
    ...
    if n_molecules > 0:
        ...
        d1_rnap_bool = np.isin(rnaps["domain_index"], domain_division["d1_all_domain_indexes"])
        d2_rnap_bool = np.isin(rnaps["domain_index"], domain_division["d2_all_domain_indexes"])
        ...
        if len(full_transcript_indexes) > 0:
            # TODO: Random state/seed in store?
            random_state = np.random.RandomState(seed=n_molecules)
            n_full_d1 = random_state.binomial(np.count_nonzero(is_full_transcript), p=0.5)
            full_d1_indexes = random_state.choice(full_transcript_indexes, size=n_full_d1, replace=False)
        ...
        partial_d1_indexes = partial_transcript_indexes[np.isin(RNAP_index_partial_transcripts, d1_rnap_indexes)]
        partial_d2_indexes = partial_transcript_indexes[np.isin(RNAP_index_partial_transcripts, d2_rnap_indexes)]
...
def follow_domain_tree(
    ...
    if children_nodes[0] != place_holder:
        ...
        # node's two children
        branches = flatten([
            follow_domain_tree(child, domain_index, child_domains, place_holder) for child in children_nodes
        ])
...
def get_descendent_domains(root_domains, domain_index, child_domains, place_holder):
    ...
    return np.array(
        flatten([
            follow_domain_tree(root_domain, domain_index, child_domains, place_holder) for root_domain in root_domains
        ])
    )
