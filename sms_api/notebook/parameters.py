...
def sample_column(rows, murein_monomers, strand_sampler, rng, shift=True):
    ...
    remaining_strand = min(murein_monomers - sum(strands), rows - sum(strands) - len(strands) - 1)
...
PARAMETER_DICT = {
    ...
    "cell_wall": {
        ...
        "upper_mean": Parameter(45, "Vollmer, W., Blanot, D., & De Pedro, M. A. (2008)."),
        ...
        "disaccharide_height": Parameter(1.03 * units.nm, "Vollmer, W., & Höltje, J.-V. (2004)."),
    ...
    "faraday_constant": Parameter(constants.value("Faraday constant") * units.C / units.mol),
...
DERIVATION_RULES = {
    ("shape", "initial_periplasm_volume"): lambda params: Parameter(
        (params.get(("shape", "initial_cell_volume")) * params.get(("shape", "periplasm_fraction"))),
    ),
    ("shape", "initial_cytoplasm_volume"): lambda params: Parameter(
        (params.get(("shape", "initial_cell_volume")) * (1 - params.get(("shape", "periplasm_fraction")))),
    ),
    ("shape", "average_periplasm_volume"): lambda params: Parameter(
        (params.get(("shape", "average_cell_volume")) * params.get(("shape", "periplasm_fraction"))),
    ),
    ("shape", "average_cytoplasm_volume"): lambda params: Parameter(
        (params.get(("shape", "average_cell_volume")) * (1 - params.get(("shape", "periplasm_fraction")))),
    ),
    ("shape", "initial_inner_area"): lambda params: Parameter(
        (np.cbrt((1 - params.get(("shape", "periplasm_fraction")))) ** 2 * params.get(("shape", "initial_outer_area"))),
    ),
    ...
    ("ampicillin", "per_porin_permeability", "outer", "ompf"): lambda params: Parameter(
        (
            ...
            / (params.get(("counts", "average_ompf")) / params.get(("shape", "average_outer_area")))
        ),
    ),
    ...
    (
        ...
        (
            params.get(("cephaloridine", "porin_specific_permeability", "outer", "ompf"))
            / (params.get(("counts", "average_ompf")) / params.get(("shape", "average_outer_area")))
        ),
    ),
    (
        ...
        (
            params.get(("cephaloridine", "porin_specific_permeability", "outer", "ompc"))
            / (params.get(("counts", "average_ompc")) / params.get(("shape", "average_outer_area")))
        ),
    ),
    (
        "cephaloridine",
        ...
        (params.get(("cephaloridine", "pbp_binding", "K_A", "PBP1A")) / params.get(("cephaloridine", "molar_mass"))).to(
            "micromolar"
        )
    ),
    (
        ...
        (params.get(("cephaloridine", "pbp_binding", "K_A", "PBP1B")) / params.get(("cephaloridine", "molar_mass"))).to(
            "micromolar"
        )
    ),
...
TABLES = {
    "Trans-membrane diffusion parameters": (
        ...
        TableRow(
            "P_{tet,outer,np}",
            param_store.get_parameter(("tetracycline", "permeability", "outer_without_porins")),
            "cm/sec",
            "Outer membrane permeability to tetracycline without porins.",
        ),
        TableRow(
            "P_{tet,outer,p}",
            param_store.get_parameter(("tetracycline", "permeability", "outer_with_porins")),
            "cm/sec",
