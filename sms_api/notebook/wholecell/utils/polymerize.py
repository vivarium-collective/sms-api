...
class polymerize(object):  # Class name is lowercase because interface is function-like
    ...
    def _gather_sequence_data(self):
        ...
        self._sequenceMonomers = np.empty((self._nMonomers, self._nSequences, self._sequenceLength), dtype=bool)
    ...
    def _prepare_running_values(self):
        ...
        self._activeSequencesIndexes = self._activeSequencesIndexes[self._sequenceReactions[:, self._currentStep]]
        self.elongation_rates = self.elongation_rates[self._sequenceReactions[:, self._currentStep]]
    ...
    def _elongate_to_limit(self):
        ...
        while notLimited and projectionIndex < self._maxElongation:
            ...
            monomerStep = sum_monomers(self._sequenceMonomers[:, :], index[active], active)
    ...
    def _finalize_resource_limited_elongations(self):
        ...
        for monomerIndex, monomerLimit in enumerate(self._monomerLimits):
            ...
            sequencesWithMonomer = np.where(
                self._sequenceMonomers[monomerIndex, self._activeSequencesIndexes, active_elongation]
            )[0]
            ...
            if nToCull > 0:
                ...
                culledIndexes = self._randomState.choice(sequencesWithMonomer, nToCull, replace=False)
        ...
        if self._reactionIsLimiting:
            ...
            if nToCull > 0:
                ...
                culledIndexes = self._randomState.choice(sequencesWithReaction, nToCull, replace=False)
    ...
    def _finalize(self):
        ...
        self.sequences_limited_elongation = (
            np.minimum(self._raw_elongation_rates, self._sequenceLength) != self.sequenceElongation
        )
