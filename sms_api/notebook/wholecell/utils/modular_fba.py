...
class FluxBalanceAnalysis(object):
    ...
    _generatedID_fractionalDifferenceBiomass = (
        "difference between fractional objective equivalents of and biomass objective - "
    )

    _generatedID_fractionsOut = "fractional objective equivalents of out - "
    _generatedID_fractionalDifferenceLeadingOut = (
        "difference between fractional objective equivalents of leading molecule and out - "
    )
    _generatedID_fractionalDifferenceBiomassOut = (
        "difference between fractional objective equivalents of and biomass objective out - "
    )
    ...
    def __init__(
        ...
        if solver not in SOLVERS:
            raise SolverUnavailableError("Unrecognized or unavailable solver: {}".format(solver))
        ...
        if objectiveParameters is not None:
            ...
            self.kinetic_objective_weight_in_range = (
                objectiveParameters.get("kinetic_objective_weight_in_range", 0) * self.kineticObjectiveWeight
            )
        else:
    ...
    def _initExternalExchange(self, externalExchangedMolecules):
        ...
        for moleculeID in externalExchangedMolecules:
            ...
            if moleculeID in externalMoleculeIDs:
                print(
                    "Warning: attempting to set multiple external exchanges" + " for {}. Skipping...".format(moleculeID)
                )
                continue
    ...
    def _initObjectiveEquivalents(self, objective, pseudoFluxFormat=None, objectiveEquivFormat=None):
    ...
    def _initObjectiveStandard(self, objective):
        ...
        for moleculeID in objective:
            ...
            self._solver.setFlowMaterialCoeff(self._standardObjectiveReactionName, objectiveEquivID, -1)
    ...
    def _initObjectiveFlexible(self, objective, objectiveParameters):
        ...
        self._solver.setFlowObjectiveCoeff(self._standardObjectiveReactionName, biomassSatisfactionWeight)
        ...
        for moleculeID in objective:
            ...
            # Biomass out
            self._solver.setFlowMaterialCoeff(self._standardObjectiveReactionName, fractionID, -1)
        ...
        for moleculeID in objective:
            ...
            self._solver.setFlowMaterialCoeff(leadingMoleculeToFractionID, fractionDifferenceLeadingID, +1)

            moleculeToFractionID = self._generatedID_moleculesToEquivalents + moleculeID

            self._solver.setFlowMaterialCoeff(moleculeToFractionID, fractionDifferenceLeadingID, -1)

            fractionDifferenceLeadingOutID = self._generatedID_fractionalDifferenceLeadingOut + moleculeID

            self._solver.flowMaterialCoeffIs(fractionDifferenceLeadingOutID, fractionDifferenceLeadingID, -1)

            self._solver.setFlowObjectiveCoeff(fractionDifferenceLeadingOutID, -fractionalDifferenceWeight)
        ...
        for moleculeID in objective:
            ...
            self._solver.setFlowMaterialCoeff(moleculeToFractionID, fractionDifferenceBiomassID, +1)

            self._solver.setFlowMaterialCoeff(fractionDifferenceBiomassID, self._standardObjectiveReactionName, -1)

            fractionDifferenceBiomassOutID = self._generatedID_fractionalDifferenceBiomassOut + moleculeID

            self._solver.setFlowMaterialCoeff(fractionDifferenceBiomassOutID, fractionDifferenceBiomassID, -1)
    ...
    def set_scaled_kinetic_objective(self, scaling=None, reactions=None):
        ...
        if scaling is None:
            ...
            raise ValueError("Not able to set a different non-zero scaling factor for only a subset of reactions")
    ...
    def _initExchangeMass(self, externalExchangedMolecules, moleculeMasses, secretionPenaltyCoeff):
        ...
        if moleculeMasses is not None:
            self._solver.setFlowMaterialCoeff(self._massExchangeOutName, self._massExchangeID, -1)
            ...
            for moleculeID in externalExchangedMolecules:
                ...
                if secretionPenaltyCoeff is not None:
                    ...
                    self._solver.setFlowMaterialCoeff(exchangeFluxID, absID, moleculeMass)

                    self._solver.setFlowMaterialCoeff(posExchangeID, absID, -1)

                    self._solver.setFlowMaterialCoeff(negExchangeID, absID, +1)

                    self._solver.setFlowObjectiveCoeff(posExchangeID, secretionPenaltyCoeff)
    ...
    def _initMaintenance(self, maintenanceCostGAM, maintenanceReaction):
        ...
        self._solver.setFlowMaterialCoeff(self._massExchangeOutName, self._pseudometaboliteGAM, maintenanceCostGAM)

        # ... which are consumed in a separate flux

        self._solver.setFlowMaterialCoeff(self._reactionID_GAM, self._pseudometaboliteGAM, -1)

        for moleculeID, stoichCoeff in maintenanceReaction.items():
            self._solver.setFlowMaterialCoeff(self._reactionID_GAM, moleculeID, stoichCoeff)

            self._solver.setFlowMaterialCoeff(self._reactionID_NGAM, moleculeID, stoichCoeff)

            self._solver.setFlowMaterialCoeff(self._reactionID_polypeptideElongationEnergy, moleculeID, stoichCoeff)
    ...
    def getKineticTargetFluxErrors(self, reactionIDs=None):
        ...
        if len(self._oneSidedReactions) > 0:
            oneSidedNegativeErrors = np.where([
                True if reactionID in self._oneSidedReactions and errors[idx] < 0 else False
                for idx, reactionID in enumerate(reactionIDs)
            ])
    ...
    def setKineticTarget(self, reactionIDs, mean_targets, lower_targets=None, upper_targets=None):
        ...
        def validate_targets(targets, reactionIDs, default=None):
            ...
            if len(targets) != len(reactionIDs):
                raise FBAError(
                    "There must be equal numbers of reactionIDs and targets when changing the kinetic target."
                )
        ...
        for reactionID, mean, lower, upper in zip(reactionIDs, mean_targets, lower_targets, upper_targets):
            if reactionID not in self._kineticTargetFluxes:
                raise FBAError(
                    ...
                    " initialized to be kinetic targets. {} is not set up for it.".format(reactionID)
                )

            if (upper < mean and not np.isclose(upper, mean)) or (mean < lower and not np.isclose(mean, lower)):
                raise FBAError(
                    "Incorrect ordering of kinetic targets: {} {} {}. Must have lower <= mean <= upper".format(
                        lower, mean, upper
                    )
                )
            ...
            if mean == 0:
                ...
                self._solver.setFlowMaterialCoeff(conversionFlux, reactionFluxEquivalent, -1)
                self.disableKineticTargets(reactionID)
            else:
                if self._currentKineticTargets[reactionID] == 0:
                    self.enableKineticTargets(reactionID)

                self._solver.setFlowMaterialCoeff(conversionFlux, reactionFluxEquivalent, -mean)
