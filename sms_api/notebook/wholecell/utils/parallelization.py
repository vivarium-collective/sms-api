...
def cpus(requested_num_processes: Optional[int] = None) -> int:
    ...
    value = os.environ.get("SLURM_CPUS_PER_TASK", os.environ.get("SLURM_JOB_CPUS_PER_NODE", os_cpus))
    ...
    if requested_num_processes is not None and requested_num_processes > 0:
        if requested_num_processes > available:
            print(
                "Warning: Request for {} worker processes got limited to {}".format(requested_num_processes, available)
            )
...
def pool(num_processes: Optional[int] = None, nestable: bool = False) -> mp.pool.Pool | InlinePool:
