...
def adjust_toya_data(
    ...
    return FLUX_UNITS * np.array([(dry_mass_frac_average * cell_density * x).asNumber(FLUX_UNITS) for x in data])
...
def get_root_to_id_indices_map(
    ...
    raise NotImplementedError(
        "This function does not provide all matches for certain reactions.Re-implement before using."
    )
...
def process_toya_data(output_ids: Iterable[str], reaction_ids: Iterable[str], data: Unum) -> Unum:
    ...
    output_data = [data_dict[output_id].asNumber(FLUX_UNITS) for output_id in output_ids]
