...
class Media(object):
    ...
    def _get_stock_media(self, raw_data):
        ...
        for label in vars(raw_data.condition.media):
            ...
            stock_media[label] = {
                row["molecule id"]: 0.0 * CONC_UNITS for row in raw_data.condition.environment_molecules
            }

            # get non-zero concentrations (assuming units.mmol / units.L)
            molecule_concentrations = getattr(raw_data.condition.media, label)

            environment_non_zero_dict = {row["molecule id"]: row["concentration"] for row in molecule_concentrations}
    ...
    def make_recipe(self, media_id):
        ...
        unitless_new_media = {mol: conc.asNumber(CONC_UNITS) for mol, conc in new_media.items()}
            ...
            if mol_id in ingredients:
                ...
                if weight is not None:
                    if weight.asNumber() == INF:
                        ...
                        raise AddIngredientsError("Negative weight given for {}".format(mol_id))
                elif mix_counts is None:
                    raise AddIngredientsError("No added added weight or counts for {}".format(mol_id))
