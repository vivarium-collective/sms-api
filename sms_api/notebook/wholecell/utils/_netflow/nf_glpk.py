...
_MAXED_OUT = (
    "GLP_EOBJ{}L: Dual simplex: The objective function being" + " maximized reached its {} limit and continues {}"
)
...
class NetworkFlowGLPK(NetworkFlowProblemBase):
    ...
    def status_string(self):
        ...
        return SOLUTION_STATUS_TO_STRING.get(self.status_code, "GLP_?: UNKNOWN SOLUTION STATUS CODE")
    ...
    def getFlowRates(self, flows):
        ...
        return np.array([self._col_primals[self._flows[flow]] if flow in self._flows else None for flow in flows])
    ...
    def getShadowPrices(self, materials):
        ...
        return np.array([
            self._row_duals[self._materialIdxLookup[material]] if material in self._materialIdxLookup else None
    ...
    def getReducedCosts(self, fluxNames):
        ...
        return np.array([
            self._col_duals[self._flows[fluxName]] if fluxName in self._flows else None for fluxName in fluxNames
        ])
    ...
    def _solve(self):
        ...
        if result != 0:
            error_str = SIMPLEX_RETURN_CODE_TO_STRING.get(result, "GLP_?: UNKNOWN SOLVER RETURN VALUE")
            print(f"{error_str}. Trying to scale problem first.")
            glp.glp_scale_prob(self._lp, glp.GLP_SF_AUTO)
            result = glp.glp_simplex(self._lp, self._smcp)
            if result != 0:
                raise RuntimeError(SIMPLEX_RETURN_CODE_TO_STRING.get(result, "GLP_?: UNKNOWN SOLVER RETURN VALUE"))
