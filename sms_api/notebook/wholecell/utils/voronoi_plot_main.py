...
def is_on_segment(p, edge):
    ...
    if (a**2 + b**2) == 0:
        ...
        if int(test * (10**9)) / (10.0**9) == 0:
            ...
            result = (x >= min(x1, x2)) and (x <= max(x1, x2)) and (y >= min(y1, y2)) and (y <= max(y1, y2))
        else:
            result = False
    ...
    def __init__(self, xy):
        ...
        def _reorder_points(corners):
            ...
            ordered_points = ordered_points[
                np.argsort(np.arctan2((ordered_points - com)[:, 1], (ordered_points - com)[:, 0]))
            ]
        ...
        for i in range(self.n_sites):
            if self.polygons[i]:
                if self.move_me[i]:  # speed-up heuristic#
                    ...
                    if dist_abs > 0:
                        ds_abs = (math.sqrt(self.polygons[i].area / math.pi)) ** 2 / (5 * dist_abs)
                ...
                # decreased.
                distance_border = self.polygons[i].find_min_dist_to_border(self.sites[i, :])
                self.weights[i] = (np.nanmin([math.sqrt(self.weights[i]), distance_border])) ** 2
    ...
    def adapt_weights(self):
        ...
        def _nearest_neighbor(self):
            ...
            nn_dist_cal = np.nanmin(
                distance_matrix(self.sites, self.sites) + np.diag([np.nan] * self.n_sites),
                axis=0,
            )
            return nn_dist_cal
    ...
    def compute_error(self):
        ...
        for i in range(self.n_sites):
            if not self.polygons[i]:  # rescue the sites if the polygon is empty
                ...
                error += abs(self.polygons[i].area - self.canvas_obj.area * self.values[i] / total_value)
            ...
            if result:
                if len(intersect_point) == 2:
                    if len(intersect_coordinates) == 0:
                        ...
                        intersect_coordinates = np.vstack([intersect_coordinates, intersect_point])

        edge = []
        if len(intersect_coordinates) > 0:
            intersect_coordinates = intersect_coordinates.reshape((-1, 2))
            intersect_coordinates = (intersect_coordinates * (10**9)).astype(int) / (10.0**9)
...
class RayClass(object):
    ...
    def is_on_ray(self, p):
        ...
        if math.isnan(test):
            ...
            if int(test * (10**9)) / (10.0**9) == 0:
                if t_x != 0:
                    result = int((p[0] - self.origin[0]) / t_x * (10**9)) / (10.0**9) >= 0

                else:
                    result = int((p[1] - self.origin[1]) / t_y * (10**9)) / (10.0**9) >= 0
    ...
    def ray_intersect_with_edge(self, edge_canvas):
        ...
        if det == 0:
            if (det_x != 0) or (det_y != 0):
                ...
                if p1_is_on_ray and p2_is_on_ray:
                    ...
                    if np.array_equal(p1, self.origin) or np.array_equal(p2, self.origin):
                        result = True
                        intersect_point = p1 * np.array_equal(p1, self.origin) + p2 * np.array_equal(p2, self.origin)
                    else:
                        result = True
                        intersect_point = [np.inf]
                else:
            ...
            x_int = det_x / det
            y_int = det_y / det
            if is_on_segment([x_int, y_int], edge_canvas) and self.is_on_ray(np.array([x_int, y_int])):
                result = True
                intersect_point = np.array([x_int, y_int])
            else:
                result = False
    ...
    def find_ray_intersect_with_canvas_and_ipoints(self, canvas_obj, ipoints, simplices_prune):
        """
        ...
        if len(intersect_coordinates) > 1:
            intersect_coordinates = self.keep_nearest_point_on_ray(intersect_coordinates)
...
class VoronoiMaster(object):
    ...
    def plot(
        ...
        if ax_shape is not None:
                ...
                for j in range(ncols):
                    ...
                    if (i == 0 and j == 0) or (chained is None):
                        (
                            ...
                            label_site_list_new,
                        ) = self._compute_boundaries(dic_current, side_length, custom_shape_vertices)
                    else:
                        (
                            voronoi_list_new,
                            polygon_value_list_new,
                    ...
                    gross_error = self._compute_error(polygon_value_list_new, total_value, total_area)
            ...
            if verbose:
                print("The error in the area representation of the wholevoronoi diagram: %.4f" % (error_all,))
            plt.title(title)
    ...
    def _compute_error(self, polygon_value_list, total_value, total_area):
        ...
        for element in polygon_value_list:
            if isinstance(element, list):
                ...
                gross_error += abs(element[0].area - total_area * element[1] / total_value)
        return gross_error
    ...
    def _compute_boundaries(
        ...
        if voronoi_list_old is not None:
            ...
            voronoi_out, error_0 = self._voronoi_main_function(labels, values, canvas_obj, voronoi_old=voronoi_old)
        else:
    ...
    def _compute_power_diagram(self, canvas_obj, sites, weights, values):
        ...
        if n_polygon < 4:
            ...
            if n_polygon == 2:
                ...
                # n_polygon = 3
                site_label = [0, 1, 2]
                sites, ray_obj_all = self._find_bisector_positive_ray(sites, weights, site_label, canvas_obj)
            ...
            ipoints, simplices_prune = self._prune_and_convert_to_ipoints(dual_sites, simplices, canvas_obj)
            ...
            polygons_all = self._divide_polygons(sites, canvas_obj, (ray_obj_all, simplices_prune, ipoints))
    ...
    def _recompute_power_diagram(self, voronoi):
        ...
        if voronoi.n_sites < 4:
            ...
            if voronoi.n_sites == 2:
                ...
                # n_polygon = 3
                site_label = [0, 1, 2]
                sites, ray_obj_all = self._find_bisector_positive_ray(sites, weights, site_label, voronoi.canvas_obj)
            ...
            ipoints, simplices_prune = self._prune_and_convert_to_ipoints(dual_sites, simplices, voronoi.canvas_obj)

            # 4. compute the ray objects of each ipoints
            ray_obj_all = self._find_multiple_positive_ray(sites, ipoints, simplices_prune)

            # 5. compute the coordinate of each polygon
            polygons_all = self._divide_polygons(sites, voronoi.canvas_obj, (ray_obj_all, simplices_prune, ipoints))
        ...
        if n_sites == 2:
            ...
            if above_below_sites[0] != above_below_sites[1]:
                for i in range(n_sites):
                    corner_polygon = canvas_obj.xy[above_below_corners == above_below_sites[i]]
            ...
            above_below_corners = np.dot(canvas_obj.xy, coefficient_matrix) + np.tile(c_r_matrix, (n_corners, 1)) >= 0
            above_below_sites = np.dot(sites, coefficient_matrix) + np.tile(c_r_matrix, (n_sites, 1)) >= 0
                ...
                for j in j_list:
                    if not len(ray_obj_all[j].edge) == 0:
                        if len(corner_polygon) == 0:
                            ...
                            corner_polygon = np.vstack([corner_polygon, ray_obj_all[j].edge])
            ...
            for i in range(n_ipoints):
                if ray_obj_all[i]:
                    ...
                    ab_corners_table[i] = (
                        np.dot(canvas_obj.xy, coefficient_matrix) + np.tile(c_r_matrix, (n_corners, 1)) >= 0
                    )
                    ab_sites_table[i] = np.dot(sites, coefficient_matrix) + np.tile(c_r_matrix, (n_sites, 1)) >= 0
            ...
            for k in range(n_sites):
                ...
                if not len(corner_polygon_all[k]) == 0:
                    ...
                    corner_polygon_all[k] = (corner_polygon_all[k] * (10**9)).astype(int) / (10.0**9)
