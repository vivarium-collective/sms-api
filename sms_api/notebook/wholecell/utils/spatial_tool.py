...
def compute_diffusion_constant_from_mw(mw, mtype=None, loc=None, temp=None, parameters=None):
    """
    ...
    dc = compute_diffusion_constant_from_rp(rp, loc=loc, temp=temp, parameters=parameters)
    return dc
...
def compute_nucleoid_size(l_cell, d_cell, length_scaling_parameters=None, nucleoid_area_ratio=None):
    """
...
def compute_n_blob(choice_model, choice_unit, bp_dna=None):
    ...
    if choice_model == "ideal":
        ...
        if choice_unit == "single":
            n_bp_per_blob = (r_blob / (3 / 8 * length_bp_conv**4 * d_dna) ** (1 / 5)) ** (5 / 3)
            n_blob = bp_dna / n_bp_per_blob
        elif choice_unit == "lp":
            ...
            length_per_unit = choice_unit * length_bp_conv
            n_unit_per_blob = (r_blob / (3 / 8 * length_per_unit**4 * d_dna) ** (1 / 5)) ** (5 / 3)
            n_bp_per_blob = choice_unit * n_unit_per_blob
            n_blob = bp_dna / n_bp_per_blob
        else:
            raise NameError(
