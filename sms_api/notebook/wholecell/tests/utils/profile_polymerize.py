...
def setup_profiler():
    ...
    polymerize._finalize_resource_limited_elongations = line_profile(polymerize._finalize_resource_limited_elongations)
    polymerize._update_elongation_resource_demands = line_profile(polymerize._update_elongation_resource_demands)

    polymerize._finalize = line_profile(polymerize._finalize)
    polymerize._clamp_elongation_to_sequence_length = line_profile(polymerize._clamp_elongation_to_sequence_length)
...
def _setupRealExample():
    ...
    sequenceLengths[np.random.choice(nSequences, nTerminating, replace=False)] = np.random.randint(
        length, size=nTerminating
    )
