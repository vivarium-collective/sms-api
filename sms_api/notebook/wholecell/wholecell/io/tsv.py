...
class reader(object):
    def __init__(
        ...
        **fmtparams: Any,
        ...
        self.reader = csv.reader(self.input_file, dialect=dialect, delimiter=delimiter, **fmtparams)
...
class writer(object):
    def __init__(
        ...
        **fmtparams: Any,
        ...
        """
        self.output_file = TextIOWrapper(csvfile, encoding="utf-8", newline="", line_buffering=True)
        self.writer = csv.writer(self.output_file, dialect=dialect, delimiter=delimiter, **fmtparams)
...
class dict_reader(object):
    def __init__(self, f: IO[bytes], fieldnames: Optional[list[str]] = None, **kwargs: Any):
        """
        ...
        self.tsv_dict_reader = csv.DictReader(tsv_reader.input_file, fieldnames=fieldnames, **kwargs)
        self.tsv_dict_reader.reader = cast(Any, tsv_reader)

        # Discard private field names that begin with underscore
        self._fieldnames = [
            fieldname for fieldname in (self.tsv_dict_reader.fieldnames or []) if not fieldname.startswith("_")
        ]
    ...
    def fieldnames(self, values: list[str]):
        ...
        self._fieldnames = [fieldname for fieldname in self.tsv_dict_reader.fieldnames if not fieldname.startswith("_")]
...
def dict_writer(f: IO[bytes], fieldnames: Collection[str], dialect: DIALECT = "excel", **kwargs: Any) -> csv.DictWriter:
    ...
    tsv_dict_writer = csv.DictWriter(tsv_writer.output_file, fieldnames, dialect=dialect, **kwargs)
