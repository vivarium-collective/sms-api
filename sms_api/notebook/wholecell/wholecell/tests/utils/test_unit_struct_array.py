...
class Test_unit_struct_array(unittest.TestCase):
    ...
    def test_getItem_slice(self):
        self.assertEqual(self.us_array[:1], UnitStructArray(self.struct_array[:1], self.units))

    def test_getItem_indicies(self):
        index = [0, 2]

        self.assertEqual(self.us_array[index], UnitStructArray(self.struct_array[index], self.units))
