...
class MemoryDebugNode(object):
    ...
    def __repr__(self):
        ...
        """
        return "{}{} 0x{:x} dict 0x{:x}".format(type(self).__name__, self.name, id(self), id(self.__dict__))
...
class Test_memory_debug(unittest.TestCase):
    def test_memory_debug(self):
        ...
        with memory_debug.detect_leaks(enabled=True):
            ...
            print("Test_memory_debug dropping refs. This should log __del__() on {}.".format(nodes[:3]))
            ...
            if PEP442:
                ...
                print(
                    ("Test_memory_debug GC'ing. {} and some of their dicts should log as uncollectable.").format(
                        uncollectable
                    )
                )
        ...
        assert len(gc.garbage) == precount + added_count, "Uncollectable: {}".format(gc.garbage)
