...
def _expected_row(index, private_field=False):
    ...
    if private_field:
        row_dict = {
            field_name: row_dict[field_name] for field_name in _remove_private_fields(FIELD_NAMES_WITH_PRIVATE_FIELD)
        }
...
class Test_Tsv(unittest.TestCase):
    ...
    def test_dict_reader_with_private_field(self):
        ...
        assert set(reader.fieldnames) == set(_remove_private_fields(FIELD_NAMES_WITH_PRIVATE_FIELD))

        reader.fieldnames = FIELD_NAMES_WITH_PRIVATE_FIELD[1:]
        assert set(reader.fieldnames) == set(_remove_private_fields(FIELD_NAMES_WITH_PRIVATE_FIELD[1:]))
