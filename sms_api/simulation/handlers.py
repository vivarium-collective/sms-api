...
async def run_workflow(
    ...
    """
    1. creates experiment id which is a modified config_id (unique)
    2. instantiates EcoliWorkflowSimulation (EcoliSimulation previously) directly.
        NOTE: This should probably instead be generated by a call to db_svc.insert_simulation. TODO: change this
    3. submits vecoli job via simulation service (slurm)
    4. Creates and returns the EcoliExperiment DTO, which contains the id with
        which users can later get simulation status/get outputs
    """
    # simulation = await database_service.insert_simulation(sim_request=simulation_request)
    experiment_id = create_experiment_id(simulation_request.config_id, simulation_request.simulator.git_commit_hash)
