...
@pytest.mark.asyncio
async def test_parca_local(
    slurm_service_local: SlurmServiceLocalHPC,
    simulation_service_local: SimulationServiceLocalHPC,
    database_service: DatabaseServiceSQL,
    latest_commit_hash: str,
) -> None:
    # check if the latest commit is already installed
    ...
    # clone the repository if needed
    await simulation_service_local.clone_repository_if_needed(
        git_commit_hash=simulator.git_commit_hash, git_repo_url=simulator.git_repo_url, git_branch=simulator.git_branch
    )

    # build the image
    job_id = await simulation_service_local.submit_build_image_job(simulator_version=simulator)
    assert job_id is not None

    start_time = time.time()
    while start_time + 60 > time.time():
        slurm_job_build = await slurm_service_local.get_job_status(slurmjobid=job_id)
        if slurm_job_build is not None and slurm_job_build.is_done():
            break
        await asyncio.sleep(5)
    ...
    # run parca
    job_id = await simulation_service_local.submit_parca_job(parca_dataset=parca_dataset)
    assert job_id is not None

    start_time = time.time()
    while start_time + 60 > time.time():
        slurm_job_parca = await slurm_service_local.get_job_status(slurmjobid=job_id)
        if slurm_job_parca is not None and slurm_job_parca.is_done():
            break
        await asyncio.sleep(5)
