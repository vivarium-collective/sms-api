...
class SpeciesChange(
    _BaseClass
):  # <-- this is done like set_species('B', kwarg=) where the inner most keys are the kwargs
    species_name: str
...
@dataclass
class GlobalParameterChange(
    _BaseClass
):  # <-- this is done with set_parameters(PARAM, kwarg=). where the inner most keys are the kwargs
    parameter_name: str
...
class SedModel(_BaseClass):
    ...
    def set_id(self, model_id=None):
        if model_id is None:
            if (
                isinstance(self.model_source, ModelFilepath)
                or isinstance(self.model_source, str)
                and "/" in self.model_source
            ):
    ...
    def get_biomodel_model_source_info(self, biomodel_id: str) -> Dict:
        ...
        return requests.get(
            url=f"https://www.ebi.ac.uk/biomodels/{biomodel_id}", headers={"accept": "application/json"}
        ).json()
...
class Experiment:
    ...
    simulation_model: Union[
        TimeCourseModel, Dict[str, Any]
    ]  # where the theory/hypothesis lies. you're stating your biological case here.
    duration: Union[
        int, float
    ]  # one level above theory model. you're sure of your case and want to not tell but show it.
