...
def run_simulation(config_path: str):
    ...
    cmd = [
        ...
        "run",
        "--env-file",
        "/Users/alexanderpatrie/Desktop/repos/ecoli/vEcoli/.env",
        "--project",
        "/Users/alexanderpatrie/Desktop/repos/ecoli/vEcoli",
        "runscripts/workflow.py",
        "--config",
        "ecoli/composites/ecoli_configs/test_installation.json",
    ]
...
async def interval_generator(
    ...
    for i, t_i in enumerate(t):
        ...
        try:
            sim.total_time = t_i
            initial_time = t[i - 1] if i != 0 else 0.0
            ...
            for mol in result_i["bulk"]:
                mol_id, mol_count = list(map(lambda _: mol.pop(0), list(range(2))))
            ...
            listener_results_i = ListenerData(
                **dict(zip(extracted_listeners, list(map(lambda name: listener_data[name], extracted_listeners))))
            )
            ...
            response_i = WCMIntervalResponse(**{
                ...
                "interval_id": str(t_i),
                "results": WCMIntervalData(bulk=bulk_results_i, listeners=listener_results_i),
            })
