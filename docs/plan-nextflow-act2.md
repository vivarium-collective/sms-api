...
**Status (2026-09-09 02:20Z): GATES 4 AND 1b ARE CLOSED; THE NEXTFLOW RUN 2 IS RUNNING.**
...
viva-api#525 **merged** (`bd034ac9`), **live on `smsvpctest` as 0.9.128** (#527, verified on the pod 01:32Z). Simulator **178** (sms-ecoli `b7e6527`) is built — the first image that can score 11/11. Run 3: bigraph-schema#187 **merged 01:57Z**; the pin bumps are eagmon's v2ecoli#748 + sms-ecoli#296 (lock-only, checks pending) — a watcher builds simulator **179** from sms-ecoli `main` the moment #296 merges, then Alex re-fires Dispatch 685 (the vEcoli-private `mecillinam>permeability>outer` provenance is a separate blocker, Chris/Mia). Still open: Phase 3 (#449), Phase 4 (ordinal identity), PBG#208, nested
