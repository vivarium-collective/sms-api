...
**Status (2026-09-09 12:50Z): gather fixes merged (v2ecoli#751 knobs, #752 one gather per variant); simulator 183 being built from the re-pin; the 739-shape before/after verification run and the Run 1 10 × 10 fire on it automatically. 742 (683's flush) is on its third gather attempt after two died staging 27 GB onto a fresh 30 GB host. Run 3's code is complete on main (#303 + #753). The MNP paths hit the gen-3 zarr bug (723, 735); `emitter: parquet` sidesteps it — the Nextflow path never used xarray.** Act 2 closed gates 4 and 1b and
