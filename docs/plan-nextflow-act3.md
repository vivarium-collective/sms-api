...
**Status (2026-09-09 03:55Z): PHASE 1 BLOCKED on cache/commit match — 725 proved the render + cache fetch, then every lineage refused the `b3b4a628` caches under image 181 (schema-3 `inputs_hash`). Needs the genotype caches rebuilt at the current commit (Alex asked).** Act 2 closed gates 4 and 1b and
